...
uint8_t BMW_KEYFOB[] = { //In fact it's 2.4Kb/s because of the Manhattan codification, see Datasheet.
...
int main(void)
{
  ...
  _spi_write_multiple(&hspi1, 0x40, BMW_KEYFOB, sizeof(BMW_KEYFOB));
  ...
  //else _spi_write_multiple(&hspi1, 0x40, HONEYWELL_KEYFOB, sizeof(HONEYWELL_KEYFOB));
    ...
    _spi_write(&hspi1, FREQ2,0x10); 
	  _spi_write(&hspi1, FREQ1,0xB0);
	  _spi_write(&hspi1, FREQ0,0x71);
